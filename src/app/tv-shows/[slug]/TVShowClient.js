    ...
    <main className="min-h-screen bg-[#020202] text-white selection:bg-primary selection:text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center w-full">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:gap-4 pt-2 md:pt-4">
                ...
                >
                  <div className="relative z-10 flex items-center gap-2 md:gap-3">
                     ...
                     <span className="uppercase tracking-widest">Watch Now</span>
                  </div>
            ...
            >
               <AnimatePresence mode="wait">
                  ...
                  >
                    <Image
                      src={tmdbImages.backdrops[currentImgIndex] || show.image_url || show.image}
      ...
      >
        <div className="container-custom flex gap-8 overflow-x-auto no-scrollbar">
          {["overview", "trailer", "photos", "episodes", "cast", "related", "reviews"].map((tab) => (
             <button
               key={tab}
            ...
            {show.trailer ? (
              ...
              <div className="flex min-h-[40vh] flex-col items-center justify-center space-y-6 rounded-3xl bg-white/5 border border-white/5 backdrop-blur-sm">
                ...
                <div className="text-center">
                  ...
                  <p className="text-zinc-500 max-w-xs">We couldn&apos;t find an official trailer for this TV show yet. Please check back later.</p>
                </div>
              </div>
            )}
          </motion.div>
          ...
          >
            {tmdbImages.backdrops.length > 0 || tmdbImages.posters.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-zinc-500">
                ...
                <p className="text-lg">No additional photos available for this TV series.</p>
              </div>
            )}
          </motion.div>
        )}
                 ...
                 <div className="flex flex-wrap gap-2">
                   {seasonNumbers.length > 0 ? seasonNumbers.map((season) => (
                     <button
                       ...
                       className={`px-4 sm:px-6 py-1.5 sm:py-2 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${
                         activeSeason === season 
                ...
                {currentEpisodes.map((ep) => (
                  ...
                  >
                    <div className="flex items-start gap-5">
                       <div className="flex-shrink-0 flex h-10 w-10 md:h-12 md:w-12 items-center justify-center rounded-full bg-zinc-800 text-white font-black text-sm md:text-lg group-hover:bg-primary group-hover:scale-110 transition-all">
                          {ep.episode_number}
                       </div>
                       <div className="flex-grow min-w-0 space-y-1 md:space-y-2">
                          <h3 className={`font-bold text-base md:text-lg truncate ${activeEpisode?.id === ep.id ? "text-primary" : "text-white group-hover:text-primary transition-colors"}`}>
               ...
               <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {relatedShows.map((m) => (
                      <div 
                        key={m.id}
                        onClick={() => {
                          router.push(`/tv-shows/${slugify(m.title)}`);
                        }}
                ...
                <p className="py-12 text-center text-zinc-500 italic">No similar shows found.</p>
             )}
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {selectedImage && (
          <motion.div
            ...
            className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 p-4 md:p-12 backdrop-blur-xl"
          >
            <motion.button
